...
import {
  ...
  Rocket, Star, Zap, ChevronRight, Clock, Loader2, Phone, Mail, Shield
} from "lucide-react";
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <main className="flex-1 space-y-6">
            {activeSection === "overview" && (
              <>
                ...
                {!user?.phoneNumbers?.length && (
                  <motion.div custom={0.25} variants={fadeUp} initial="hidden" animate="visible" className="border border-yellow-500/20 rounded-2xl bg-yellow-500/5 p-4">
                    <p className="text-sm text-yellow-400 flex items-center gap-2">
                      <Phone className="w-4 h-4" />
                      Add a phone number in{" "}
                      <button onClick={() => setActiveSection("settings")} className="underline hover:text-yellow-300">Settings</button>{" "}
                      to receive order tracking updates.
                    </p>
                  </motion.div>
                )}
            ...
            {activeSection === "orders" && (
              <motion.div custom={0.2} variants={fadeUp} initial="hidden" animate="visible" className="border border-white/10 rounded-2xl bg-white/5 overflow-hidden">
                ...
                {ordersLoading ? (
                  ...
                  <div className="divide-y divide-white/5">
                    {orders.map((order) => {
                      const colors = statusColors[order.status] ?? statusColors.pending;
                      return (
                        <Link key={order.id} href={`/dashboard/orders/${order.id}`} className="flex items-center justify-between p-5 hover:bg-white/5 transition-colors">
                          <div>
                            <p className="font-bold text-white">Mission #{order.order_number}</p>
                          ...
                          <div className="flex items-center gap-4">
                            <span className={`text-xs font-mono px-3 py-1 rounded-full ${colors.bg} ${colors.text}`}>{order.status}</span>
                            <span className="text-white font-bold">{formatPrice(order.total)}</span>
                          </div>
                        </Link>
                      );
...
function WishlistSection({ userId }: { userId: string }) {
  ...
  if (isLoading) return <div className="flex items-center justify-center py-12"><Loader2 className="w-6 h-6 text-orange-400 animate-spin" /></div>;
  ...
  return (
    <motion.div custom={0.2} variants={fadeUp} initial="hidden" animate="visible" className="border border-white/10 rounded-2xl bg-white/5 overflow-hidden">
      ...
      {items.length === 0 ? (
        <div className="text-center py-12">
          <div className="text-4xl mb-3">⭐</div>
          <p className="text-gray-500">Your wishlist is empty. Explore the fleet!</p>
          <Link href="/products"><Button size="sm" className="mt-4 bg-orange-600 hover:bg-orange-500 rounded-full">Browse the Fleet</Button></Link>
        </div>
      ) : (
        <div className="divide-y divide-white/5">
          {items.map((item) => (
            <div key={item.id} className="flex items-center justify-between p-5">
              ...
              <div className="flex items-center gap-2">
                <Link href={`/products/${item.product.id}`}><Button variant="ghost" size="sm" className="text-orange-400 hover:text-orange-300">View</Button></Link>
                <Button variant="ghost" size="sm" className="text-red-500 hover:bg-red-500/10" onClick={() => toggle(userId, item.product)}>Remove</Button>
              </div>
            </div>
          ))}
        </div>
...
function SettingsSection({ user }: { user: ReturnType<typeof useUser>["user"] | null | undefined }) {
  ...
  return (
    <motion.div custom={0.2} variants={fadeUp} initial="hidden" animate="visible" className="space-y-4">
      <div className="border border-white/10 rounded-2xl bg-white/5 p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <div className="flex-1 flex gap-2">
              <input
                ...
                className="flex-1 bg-black/50 border border-white/10 rounded-lg px-3 py-1.5 text-white text-sm focus:border-orange-500 outline-none"
              />
              <button
                onClick={updatePhone}
                disabled={saving}
                className="px-4 py-1.5 text-sm font-medium bg-orange-600 hover:bg-orange-500 disabled:opacity-50 rounded-lg text-white transition-colors"
              >
                {saving ? "..." : "Save"}
              </button>
            </div>
          ...
          <p className="text-xs text-gray-500 mt-4">
            Phone is used for order tracking updates. To require phone during sign-up, enable it in the{" "}
            <a href="https://dashboard.clerk.com" target="_blank" rel="noopener noreferrer" className="text-orange-400 hover:underline">Clerk Dashboard</a>{" "}
            under User & Authentication → Email, Phone, Username → Phone.
          </p>
        </div>
      </div>
      <div className="border border-white/10 rounded-2xl bg-white/5 p-6">
        <h3 className="font-bold text-white text-sm mb-2">Account Security</h3>
