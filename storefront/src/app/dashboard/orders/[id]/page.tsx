...
export default function OrderDetailPage() {
  ...
  useEffect(() => {
    const userId = "user-id-placeholder"; // Will be set from Clerk
    Promise.all([
      fetch(`/api/orders?userId=${userId}`).then((r) => r.json()),
      fetch(`/api/orders/${id}/tracking`).then((r) => r.json()),
    ]).then(([oData, tData]) => {
      const found = (oData.orders ?? []).find((o: Order) => o.id === id);
      setOrder(found ?? null);
  ...
  const currentIdx = TRACKING_STAGES.findIndex((s) => s.status === order.status) ?? -1;
